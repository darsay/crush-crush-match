...
public class DailyRewardButton : MonoBehaviour
{
    ...
    string _dailyRewardGotName = "DailyRewardGot";
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.HasKey(_dailyRewardName))
        {
            ...
            {
                PlayerPrefs.SetInt(_dailyRewardGotName, 0);
            }
        }
        else
        {
        ...
        if (PlayerPrefs.HasKey(_dailyRewardGotName))
        {
            if (PlayerPrefs.GetInt(_dailyRewardGotName) > 0)
            {
    ...
    async void WatchAdAndContinue()
    {
        if (await ServiceLocator.GetService<AdsGameService>().ShowAd())
        {
            ...
            PlayerPrefs.SetInt(_dailyRewardGotName, 1);
